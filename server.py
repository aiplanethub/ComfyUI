...
class PromptServer():
    def __init__(self, loop):
        ...
        def image_upload(post, image_save_function=None):
            ...
            allowed_file_types = ['.txt', '.pdf', '.docx', '.pptx', '.md', '.csv', '.xlsx']
            ...
            if image and image.file:
                ...
                if file_extension not in allowed_file_types:
                    return web.Response(status=415, text=f"Unsupported file type: {file_extension}. Allowed types: {', '.join(allowed_file_types)}")
                ...
                if overwrite is not None and (overwrite == "true" or overwrite == "1"):
                    ...
                    while os.path.exists(filepath):
                        if compare_image_hash(filepath, image):  # Compare hash to prevent saving duplicates with same name
